...
# -*- coding: utf-8 -*-
##########################################
# 출처: http://pinkwink.kr/1005 [PinkWink]
##########################################
import numpy as np
...
def drawKorea(target_data, blocked_map, d1, d2, cmapname):
    ...
    white_label_min = (max(blocked_map[target_data]) - min(blocked_map[target_data])) * 0.25 + min(blocked_map[target_data])
    ...
    masked_mapdata = np.ma.masked_where(np.isnan(map_data), map_data)
